 					...
 					<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>$pro_price.00
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger bn-xs'>AddToCart</button>	
								</div>								
							</div>
							</div>
 				";
...
if(isset($_POST['addToProduct'])){
	...
	$sql= "SELECT * FROM `cart` WHERE `ip_id`='$p_id' AND `user_id`='$user_id'";
	$run_query = mysqli_query($conn,$sql);
	...
	if($count > 0){
		...
		$row = mysqli_fetch_array($run_query);
			...
			$pro_image=$row['product_image'];
			$pro_price = $row['product_price'];
		$sql = "INSERT INTO `cart` (`id`,`ip_id`,`ip_add`,`user_id`,`product_title`,`product_image`,`qty`,`price`,`total_amount`)
